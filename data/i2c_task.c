...
static void ISR_i2c_handler() {
	...
	vTaskNotifyGiveFromISR(taskHandle,
...
void i2c_read(uint8_t addr, uint8_t *buffer, uint32_t len, uint8_t op) {
	i2c_struct_t  i2c;
	uint8_t *cmd;
...
void i2c_write (uint8_t addr, uint8_t *data, uint32_t len, uint8_t op) {
	i2c_struct_t i2c;
	uint8_t *cmd;
	int i =  0;
...
uint8_t i2c_16read8 (uint8_t dev_addr, uint16_t reg_addr) {
	...
	i2c_write (dev_addr,(uint8_t*) &swapped_addr, 2, 0x0);
